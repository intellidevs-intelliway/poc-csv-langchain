...
class MongoChain:
    def detect_intent(self, query: str = "") -> str:
        ...
        # return res
        query_res = self.run_query(self=MongoChain, connection_string=os.getenv("MONGO_CONNECTION_STRING"), command=res.strip(), input=query)
    ...
    def run_query(self, connection_string: str = "", command: str = "", input: str = "") -> str:
        ...
        temp = self.evaluate_response(self=MongoChain, evaluate=res, user_query=input)
...
if __name__ == '__main__':
    # res = MongoChain.detect_intent(self=MongoChain, query="What's the price of Smarthpone 1?")
    res = MongoChain.run_query(self=MongoChain, connection_string=os.getenv("MONGO_CONNECTION_STRING"), command="collection.find_one({'name': 'Smartphone 1'})", input="")
    print(res)
